...
class Environment(gym.Env):
    ...
    def step(self, action):      
        ...
        if np.array_equal(observation, self.observation_one_hot[1]):      # 前方沒車且車子在路線上方
            ...
            if action == 1:
                reward += 1
            if action == 2:   # left（錯方向）
                reward -= 100
            if action == 3:   # right（修正）
                reward += 14
            if action == 4:
                reward -= 6
        ...
        if np.array_equal(observation, self.observation_one_hot[2]):      # 前方沒車且車子在路線下方
            ...
            if action == 2:   # left（修正）
                reward += 14
            if action == 3:   # right（錯方向）
                reward -= 100
            if action == 4:
                reward -= 6


        if np.array_equal(observation, self.observation_one_hot[3]):      # 上下前都有車（高風險）
            if action == 0:   # brake（最安全）
                reward += 20
            if action == 1:
                reward -= 25
            if action == 2:
